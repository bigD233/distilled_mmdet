...
class MultiBranchLabelTwoStageDetector(BaseDetector):
    ...
    def _load_from_state_dict(self, state_dict: dict, prefix: str,
        ...
        if not is_substring_of_key(state_dict, 'backbone_ir'):
            ...
            for k, v in ori_dict.items():
                ...
                if len(k.split('.')) >= 4 and k.split('.')[3] == 'instance_interactive_conv':
                    state_dict[k.replace('instance_interactive_conv', 'instance_interactive_conv_ir')] = v
                if len(k.split('.')) >= 4 and k.split('.')[3] == 'instance_interactive_conv_norm':
                    state_dict[k.replace('instance_interactive_conv_norm', 'instance_interactive_conv_norm_ir')] = v
            del ori_dict
    ...
    def _forward(self, batch_inputs: Tensor,
        ...
        roi_outs = self.roi_head.forward([x, x_ir], rpn_results_list,
                                         batch_data_samples)
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        roi_losses = self.roi_head.loss([x, x_ir], rpn_results_list,
                                        batch_data_samples)
    ...
    def predict(self,
        ...
        results_list = self.roi_head.predict(
            [x, x_ir], rpn_results_list, batch_data_samples, rescale=rescale)
        for res in results_list:
            for i, label in enumerate(res['labels']):
                if label > self.cat_id:
                    res['labels'][i] = self.cat_id
