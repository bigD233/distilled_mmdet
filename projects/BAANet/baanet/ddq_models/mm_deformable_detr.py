...
from .mm_base_detr import MultiModalDetectionTransformer


@MODELS.register_module()
class MultiModalDeformableDETR(MultiModalDetectionTransformer):
    ...
    def pre_transformer(
        ...
        encoder_inputs_dict = dict(
            feat=feat_flatten, # [2, 7953, 256]
            feat_mask=mask_flatten,
            feat_pos=lvl_pos_embed_flatten,
            spatial_shapes=spatial_shapes,
            level_start_index=level_start_index,
            valid_ratios=valid_ratios)
        decoder_inputs_dict = dict( # same for two modes
    ...
    def gen_encoder_output_proposals_ir(
            self, memory: Tensor, memory_mask: Tensor,
            spatial_shapes: Tensor) -> Tuple[Tensor, Tensor]:
        """Generate proposals from encoded memory. The function will only be
        used when `as_two_stage` is `True`.
        ...
        output_memory = output_memory.masked_fill(~output_proposals_valid,
                                                  float(0))
        output_memory = self.memory_trans_fc_ir(output_memory)
        output_memory = self.memory_trans_norm_ir(output_memory)
        # [bs, sum(hw), 2]
        return output_memory, output_proposals
