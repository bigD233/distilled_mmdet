...
def multi_scale_deformable_attn_pytorch(
    ...
    attention_weights = attention_weights.transpose(1, 2).reshape( # [8, 1, 2396, 16]
        bs * num_heads, 1, num_queries, num_levels * num_points)
    output = (torch.stack(sampling_value_list, dim=-2).flatten(-2) * # [8, 32(embed_dims), 2396, 16]
...
class MultiModalMultiScaleDeformableAttention(BaseModule):
    ...
    def __init__(self,
        ...
        self.attention_weights_ir = nn.Linear(embed_dims,
                                           num_heads * num_levels * num_points)
    ...
    @deprecated_api_warning({'residual': 'identity'},
                            cls_name='MultiModalMultiScaleDeformableAttention')
    def forward(self,
                query: torch.Tensor,
        ...
        bs, num_query, _ = query.shape # [1, 2396, 256]
        bs, num_value, _ = value.shape # [1, 15906, 256]
        assert (spatial_shapes[:, 0] * spatial_shapes[:, 1]).sum() == num_value / 2
        ...
        if p < 1/3: # only rgb
            ...
            sampling_offsets_rgb = self.sampling_offsets(query).view( # [1, 2396, 8, 4, 4, 2]
                bs, num_query, self.num_heads, self.num_levels, self.num_points, 2)
            attention_weights_rgb = self.attention_weights(query).view( # [1, 2396, 8, 4*4]
                bs, num_query, self.num_heads, self.num_levels * self.num_points)
            sampling_offsets_ir = self.sampling_offsets_ir(query).view( # [1, 2396, 8, 4, 4, 2]
                bs, num_query, self.num_heads, self.num_levels, self.num_points, 2)
        ...
        attention_weights = attention_weights.view(bs, num_query, # [1, 2396, 8, 8, 4]
                                                   self.num_heads,
                                                   -1, # self.num_lvls
                                                   self.num_points
                                                   ) 
