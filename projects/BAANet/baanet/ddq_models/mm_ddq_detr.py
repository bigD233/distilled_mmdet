...
class MultiModalDDQDETR(MultiModalDINO):
    ...
    def _init_layers(self) -> None:
        super(MultiModalDDQDETR, self)._init_layers()
    ...
    def forward_decoder(self,
        ...
        if p < 1/3: # only rgb
            inter_states, references = self.decoder( # [6, 2, 2448, 256], [7, 2, 2448, 4]
                query=query, # [2, 2448, 256]
                value=memory, # [2, 7953, 256]
                key_padding_mask=memory_mask, # [2, 7953]
                self_attn_mask=dn_mask, # [2448, 2448]
                reference_points=reference_points, # [2, 2448, 4]
                spatial_shapes=spatial_shapes, # [4, 2]
                level_start_index=level_start_index,# [4]
                valid_ratios=valid_ratios, # [2, 4, 2]
                reg_branches=self.bbox_head.reg_branches)
        elif p >= 1/3 and p < 2/3: # only ir
            inter_states, references = self.decoder( # [6, 2, 2448, 256], [7, 2, 2448, 4]
                query=query, # [2, 2448, 256]
                value=memory_ir, # [2, 7953, 256]
                key_padding_mask=memory_mask, # [2, 7953]
                self_attn_mask=dn_mask, # [2448, 2448]
                reference_points=reference_points, # [2, 2448, 4]
                spatial_shapes=spatial_shapes, # [4, 2]
            ...
            inter_states, references = self.decoder( # [6, 2, 2448, 256], [7, 2, 2448, 4]
                ...
                reg_branches=self.bbox_head.reg_branches)
            inter_states, references = self.decoder( # [6, 2, 2448, 256], [7, 2, 2448, 4]
                query=inter_states[-1], # [2, 2448, 256]
                value=memory_ir, # [2, 7953, 256]
                key_padding_mask=memory_mask, # [2, 7953]
                self_attn_mask=dn_mask, # [2448, 2448]
                reference_points=references[-1], # [2, 2448, 4]
                spatial_shapes=spatial_shapes, # [4, 2]
                level_start_index=level_start_index,# [4]
                valid_ratios=valid_ratios, # [2, 4, 2]
                reg_branches=self.bbox_head.reg_branches)
