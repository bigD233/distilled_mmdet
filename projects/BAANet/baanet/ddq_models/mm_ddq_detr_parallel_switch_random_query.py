...
class RandomQueryParallelMultiModalDDQDETR(MultiModalDINO):
    ...
    def init_weights(self) -> None:
        ...
        nn.init.xavier_uniform_(self.dense_query_embedding.weight)
        normal_(self.level_embed)

    def _init_layers(self) -> None:
        super(RandomQueryParallelMultiModalDDQDETR, self)._init_layers()
        ...
        self.dense_query_embedding = nn.Embedding(self.num_queries, self.embed_dims)
    ...
    def pre_decoder(
        ...
        for img_id in range(num_imgs):
            ...
            if self.training:
                ...
                dense_query = self.dense_query_embedding.weight[:, None, :]
                dense_query = dense_query.repeat(1, bs, 1).transpose(0, 1)
        ...
        if self.training:
            ...
            # NOTE static ref pts
            topk_coords_unact = output_proposals[:, ::7, :][:, :1000, :]
    ...
    def forward_decoder(self,
        ...
        all_query = query
                # torch.cat([query[:, :num_dn, :], # dn query
        ...
        all_ref_pts = reference_points
                        # torch.cat([reference_points[:, :num_dn, :], # dn reference_points
