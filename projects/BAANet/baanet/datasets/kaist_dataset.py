...
class KAISTDataset(BaseDetDataset):
    ...
    def parse_data_info(self, raw_data_info: dict) -> Union[dict, List[dict]]:
        ...
        # if img_info.get('file_name') and len(img_info['file_name'].split('/')) < 5:            
        #         img_path = self.data_prefix['img_path'] + '/kaist_test_lwir/'  + img_info['file_name'] + '.png'
        # else: 
        if img_info.get('file_name'):
            img_path = osp.join(self.data_prefix['img_path'], img_info['file_name'])
        else:
            img_path = osp.join(self.data_prefix['img_path'], img_info['im_name'].replace('/','_'))
