...
class TwoStageFGDDetector(BaseDetector):
    ...
    def __init__(self,
        ...
        if self.distill:
            self.teacher_backbone , self.teacher_neck = self._load_distilled_weights(teacher_cfg,teacher_pretrained)
            ...
            for item_loc in distill_cfg:
                ...
                # print('teacher_'+item_loc.teacher_module,item_loc.student_module)
                teacher_modules['teacher_'+item_loc.teacher_module].register_forward_hook(hook_teacher_forward)
                # print(teacher_modules['teacher_'+item_loc.teacher_module])
    ...
    def _load_distilled_weights(self,teacher_cfg,teacher_pretrained,device: str = 'cuda:0'):
        ...
        load_checkpoint(model_backbone, teacher_pretrained, map_location='cpu',revise_keys=[(r'^backbone\.', '')])
        load_checkpoint(model_neck, teacher_pretrained, map_location='cpu',revise_keys=[(r'^neck\.', '')])
    ...
    def extract_teacher_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        Returns:
            ...
            different resolutions.
        """
        x_t = self.teacher_backbone(batch_inputs)
        if self.with_neck:
        ...
        if self.distill:
            ...
            for item_loc in self.distill_cfg:
                ...
                for item_loss in item_loc.methods:
                    ...
                    distill_losses[loss_name] = self.distill_losses[loss_name](student_feat,teacher_feat,batch_data_samples)
