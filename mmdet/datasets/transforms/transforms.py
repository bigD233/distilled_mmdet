...
import mmdet.datasets.transforms.mlpd_functional as F
import cv2
import mmengine
import mmcv
import numpy as np
...
@TRANSFORMS.register_module()
class MultiModalRandomFlip(BaseTransform):
    def __init__(self,
                 prob: Optional[Union[float, Iterable[float]]] = None,
                 flip: Optional[Union[float, Iterable[float]]] = None) -> None:
        self.prob = prob
        self.flip = flip
    ...
    def _flip_on_direction(self, results: dict) -> None:
        ...
        keypoints."""
        pair = 1
        results['gt_bboxes_rgb'] = copy.deepcopy(results['gt_bboxes'])
        results['gt_bboxes_ir'] = copy.deepcopy(results['gt_bboxes'])
        results['gt_bboxes_labels_rgb'] = copy.deepcopy(results['gt_bboxes_labels'])
        results['gt_bboxes_labels_ir'] = copy.deepcopy(results['gt_bboxes_labels'])
        results['flip'] = False
        ...
        if random.random() < self.prob:
            ...
            results['flip'] = True
            # flip rgb image
            results['img'][:, :, :3]= mmcv.imflip(
                results['img'][:, :, :3], direction=results['flip_direction'])
            img_shape = results['img'].shape[:2]
            # flip bboxes
            if results.get('gt_bboxes_rgb', None) is not None:
                results['gt_bboxes_rgb'].flip_(img_shape, results['flip_direction'])
            pair = 0
            if random.random() < self.flip:      
                ...
                img_shape = results['img'].shape[:2]
                # flip bboxes
                if results.get('gt_bboxes_ir', None) is not None:
                    results['gt_bboxes_ir'].flip_(img_shape, results['flip_direction'])
                pair = 1
        if pair == 1 :
            results['gt_bboxes_labels'][:] = 0 # 1, 1
        else : 
            if len(results['gt_bboxes_rgb']) != 0 :
                results['gt_bboxes_labels_rgb'][:] = 1 #  1, 0
            if len(results['gt_bboxes_ir']) != 0 :
                results['gt_bboxes_labels_ir'][:] = 2 # 0, 1
            results['gt_bboxes'] = HorizontalBoxes.cat([results['gt_bboxes_rgb'], results['gt_bboxes_ir']])
            results['gt_bboxes_labels'] = np.concatenate([results['gt_bboxes_labels_rgb'], results['gt_bboxes_labels_ir']], axis=0)
            results['gt_ignore_flags'] = np.concatenate([results['gt_ignore_flags'], results['gt_ignore_flags']])
...
@TRANSFORMS.register_module()
class MultiModalYOLOXHSVRandomAug(BaseTransform):
    def __init__(self,
                 hue_delta: int = 5,
                 saturation_delta: int = 30,
                 value_delta: int = 30,
...
class RandomMask(BaseTransform):
    def __init__(self,
                 prob = 0.5,
                 mask_type = 'noise',
                 mask_modality = 'IR') -> None:
        self.prob = prob
        assert mask_type in ['noise', 'white', 'black']
        self.mask_type = mask_type
        assert mask_modality in ['RGB', 'IR']
