...
class ReasonableCOCOEval(COCOeval):
    def _prepare(self):
        '''
        ...
        for gt in gts:
            ...
            gt['height'] = gt['bbox'][3] if 'height' not in gt else gt['height']
            gt['occlusion'] = 0 if 'occlusion' not in gt else gt['occlusion']
            gt['iscrowd'] = gt['iscrowd'] if 'iscrowd' in gt else 0
            gt['area'] = gt['bbox'][-1] * gt['bbox'][-2]
            gt['segmentation'] = []
            ...
            gt['ignore'] = 1 \
                if gt['height'] < 55 \
                or gt['height'] > 1e5 ** 2 \
                or gt['occlusion'] not in [0, 1] \
                or gbox[0] < 5 \
                or gbox[1] < 5 \
                or gbox[0] + gbox[2] > 635 \
                or gbox[1] + gbox[3] > 507 \
                else gt['ignore']
...
class ReasonableCocoMetric(BaseMetric):
    ...
    """
    default_prefix: Optional[str] = 'reasonable_coco'
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for metric in self.metrics:
            ...
            coco_eval = ReasonableCOCOEval(self._coco_api, coco_dt, iou_type)
